    ...
    <section id="contact" className="py-24 bg-background">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          <p className="text-sm font-semibold uppercase tracking-widest text-gold mb-3">Contact Us</p>
          ...
          {details.map((d) => (
            <div key={d.label} className="flex items-start gap-4 bg-card rounded-xl p-6 border border-border">
              ...
              <div>
                ...
                {d.href ? (
                  <a href={d.href} className="text-foreground hover:text-gold transition-colors text-sm" target={d.label === "Website" ? "_blank" : undefined} rel={d.label === "Website" ? "noopener noreferrer" : undefined}>
                    {d.value}
                  </a>
                ) : (
